...
\title{Advice on constructing composite ordinal outcomes}
...
\begin{abstract}
  ...
  outcomes---into an ordinal composite: (1) overriding (or ``sequential
  composition''), (2) tie-breaking (or ``hierarchical composition''), and what
  we term (3) ``parallel composition''. We illustrate each of these mechanisms
  with examples from the literature. Next, we discuss how constructing
  fine-grained composite outcomes can improve validity and efficiency---but
  also why adding levels sometimes has very little effect on efficiency.
  Finally, we connect these general methods for combining ordinal outcomes with
  the ``prioritized outcomes'' literature---including win-based and
  desirability of outcome ranking (DOOR) methodologies---clearly delineating
  the task of constructing composite ordinal outcomes from the related but
  distinct tasks of choosing effect measures, choosing estimators, and handling
  censoring.
...
meaningful. Ordinal outcomes have a long history in specific disease
areas---for example the modified Rankin scale in stroke trials---and have
become increasingly prominent in clinical resarch following the COVID-19
pandemic when disease-specific ordinal outcome scales were developed by the WHO
and NIAID \citep{broderickEvolutionModifiedRankin2017,
beigelRemdesivirTreatmentCovid192020, marshallMinimalCommonOutcome2020}.
...
\emph{efficiency}. \emph{Valid} outcomes reflect differences in patient benefit
and harm; \emph{efficient} outcomes are able to discriminate among better- and
worse-off patients with relatively small sample sizes. (Each of these is a
function of other factors too.)

Ordinal outcomes have the potential to
...
\section{Three ways to combine existing ordinal scales}
...
\subsection{More levels: increasing efficiency by
shrinking the largest category proportion}
...
\subsection{More levels: Consider efficiency within levels of strong predictors}

\begin{figure}
  \includegraphics[width=7in]{stratified_efficiency_plots.png}
  \caption{\textbf{Overriding (sequential composition) illustrated
    with the PASSPORT outcome} Comparison of the ordinal outcome used
  in PASSPORT (depicted in Figure \ref{fig:passport_outcome})} with a
  coarsened 4-level version, stratified by Australisian Triage Scale category.
  \label{fig:stratified_efficiency}
\end{figure}

\subsection{Fewer levels: avoid comparing ambiguous levels (validity)}
...
\subsection{Fewer levels: ease of communication}
...
\section{Composite ordinal outcomes and ``win'' methods}
